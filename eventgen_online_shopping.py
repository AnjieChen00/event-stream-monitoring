...
for uid in uids:
	...
	place_order = {"event_type_name": "place_order", "user_id": str(uid), 'order_id': str(oid), 'item_id_quantity_mapping': default_item_id_quantity_mapping, 'payment_method': 'apple pay', 'payment_amount': random.randint(90,10e5)/100,'payment_tracking_id': str(pid), "event_report_time": z0}
	z1 = z0 + np.clip((int(xmax*(0.5+np.random.normal(0,1)))),1,xmax-1)
	picking = {"event_type_name": "picking", 'order_id': str(oid), 'item_id': str(iid), 'warehouse_id': str(wid), "event_report_time": z1}
	z2 = z1 + np.clip((int(ymax*(0.5+np.random.normal(0,1)))),1,ymax-1)
	packing = {"event_type_name": "packing", 'order_id': str(oid), 'package_id': str(pid), 'item_id_quantity_mapping': default_item_id_quantity_mapping, 'warehouse_id': str(wid), "event_report_time": z2}
	z3 = z2 + np.clip((int(ymax*(0.5+np.random.normal(0,1)))),1,ymax-1)
	assign_carrier = {"event_type_name": "assign_carrier", 'package_id': str(pid), 'warehouse_id': str(wid), 'carrier_id': str(cid), "event_report_time": z3}
	z4 = z3 + np.clip((int(zmax*(0.5+np.random.normal(0,1)))),1,zmax-1)
	print_shipping_label = {"event_type_name": "print_shipping_label", 'package_id': str(pid), 'warehouse_id': str(wid), 'carrier_id': str(cid), 'tracking_number': str(tid), "event_report_time": z4}
	ship = {"event_type_name": "ship", 'package_id': str(pid), 'warehouse_id': str(wid), 'carrier_id': str(cid), 'tracking_number': str(tid), "event_report_time": z4+1}
	z5 = z4 + np.clip((int(mmax*(0.5+np.random.normal(0,1)))),1,mmax-1)
	deliver = {"event_type_name": "deliver", 'package_id': str(pid), 'carrier_id': str(cid), 'tracking_number': str(tid), "event_report_time": z5}
	confirm_delivery = {"event_type_name": "confirm_delivery", 'package_id': str(pid), 'carrier_id': str(cid), 'tracking_number': str(tid), "event_report_time": z5+1}	
